<div class="md:grid md:grid-cols-3 md:gap-6 pt-10 sm:pt-0">
    ...
    <div class="md:col-span-1 flex justify-between">
        <div class="px-4 sm:px-0">
            ...
            @else
                @if ($solicitud)
                    @if ($solicitud->manda == $yo->id)
                        <p class="mt-1 text-sm text-gray-600">
                            Usted ha enviado una solicitud
                        </p>
                    @else
                        <p class="mt-1 text-sm text-gray-600">
                            Usted ha recibido una solicitud
                        </p>
                    @endif
                @else
                    <p class="mt-1 text-sm text-gray-600">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <div>
                    ...
                    <div
                        ...
                        <x-jet-button wire:click="cancelar" wire:loading.attr="disabled">
                            Cancelar
                        </x-jet-button>
                    </div>
                </div>
                                    ...
                                    <ul class="space-y-1 mt-2">
                                        <li class="py-3 rounded-lg bg-white text-gray-600 text-left">
                                            @if ($user)
                                                ...
                                                <p><span class="font-bold">Genero:</span>
                                                    {{ $user->genero == 'M' ? 'MUJER' : 'HOMBRE' }}</p>
                                                <p><span class="font-bold">Equipo:</span>
                                                    {{ count($user->descendants) }}
                                                    personas</p>
                                                <p><span class="font-bold">Celulas Oficiales:</span>
                                                    {{ count($user->recursiveCelulasTodas) }}</p>
                                                <p><span class="font-bold">Celulas Evangelisticas:</span>
                                                    {{ count($user->recursiveEvangelisticasTodas) }} </p>
                                                <p><span class="font-bold">Visitas Pendientes CE:</span>
                                                    {{ $visitas_pendientes }} </p>
                                                <p><span class="font-bold">CE Visitadas:</span> {{ $ce_visitadas }}
                                                </p>
                                            @endif
                                        </li>
                                    </ul>
