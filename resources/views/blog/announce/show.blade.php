...
@section('main')
    <div class="bg-white font-sans leading-normal tracking-normal">
                ...
                <div class="flex flex-wrap items-center content-center">
                    ...
                    <div class="flex w-1/2 justify-end content-center">		
                        <p class="hidden sm:block mr-3 text-center h-14 p-4 text-xs"><span class="pr-2">Comparte esto</span> 👉</p>
                            <a class="inline-block text-white no-underline hover:text-white hover:text-underline text-center h-10 w-10 p-2 md:h-auto md:w-16 md:p-4" href="https://twitter.com/intent/tweet?url={{route('blog.show_announces',$anuncio->slug)}}" style="background-color:#33b1ff;">
                            ...
                            <a class="inline-block text-white no-underline hover:text-white hover:text-underline text-center h-10 w-10 p-2 md:h-auto md:w-16 md:p-4" href="https://www.facebook.com/sharer/sharer.php?u={{route('blog.show_announces',$anuncio->slug)}}" style="background-color:#005e99">
        ...
        <div class="md:hidden mx-auto container px-2 lg:px-8  mt-4 bg-cover ">
            <img class="w-full h-96"src="@if ($anuncio->image) {{ asset('storage/' . $anuncio->image->url) }}@else https://pbs.twimg.com/profile_images/740993726189834240/WbUqIPMS.jpg @endif" alt="">
        </div>
        ...
        <div class="container max-w-5xl mx-auto md:-mt-32">
            <div class="mx-0 sm:mx-6">
                <div class="bg-white w-full p-8 md:p-24 text-xl md:text-2xl text-gray-800 leading-normal" style="font-family:Georgia,serif;">
                    ...
                    <blockquote class="pt-4 border-l-4 border-green-500 italic my-8 pl-8 md:pl-12">{{$anuncio->extract}}</blockquote>
                ...
                <div class="flex w-full items-center font-sans p-8 md:p-24">
                    <img class="w-10 h-10 rounded-full mr-4" src="@if($anuncio->image){{asset('storage/'.$anuncio->image->url)}}@else https://pbs.twimg.com/profile_images/740993726189834240/WbUqIPMS.jpg @endif" alt="Avatar of Author">
                    <div class="flex-1">
                    ...
                    <div class="justify-end">
                        <a href="{{route('blog.announces')}}" class="bg-transparent border border-gray-500 hover:border-green-500 text-xs text-gray-500 hover:text-green-500 font-bold py-2 px-4 rounded-full">Leer Mas</a>
                    </div>
                </div>
            </div>  
        </div>
                    ...
                    @foreach ($similares as $announce)
                        <div class="pt-4 grid-cols-2 shadow mt-5 text-sm relative max-w-64 border-0  rounded-lg break-words text-gray-800 flex flex-col" style="background-color:white;">
                            ...
                            <div class="lg:hidden px-6 pb-2">
                                <span class=" float-right font-bold text-sm text-gray-400">{{$announce->created_at->toFormattedDateString()}}</span>                             
                            </div>
                        </div>
                    @endforeach
                </div>
